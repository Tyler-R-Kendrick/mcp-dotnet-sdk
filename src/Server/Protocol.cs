...
namespace Server;
    ...
    public interface IProtocol<TResult, TNotification>
        : IDisposable
    {
        event Action OnClose;
        event Action<Exception> OnError;

        Channel<TNotification> Notifications { get; }
...
//         private readonly Dictionary<string, Func<JSONRPCRequest, RequestHandlerExtra, Task<SendResultT>>> _requestHandlers = [];
//         private readonly Dictionary<string, AbortController> _requestHandlerAbortControllers = [];
//         private readonly Dictionary<string, Func<JSONRPCNotification, Task>> _notificationHandlers = [];
//         private readonly Dictionary<int, Action<JSONRPCResponse>> _responseHandlers = [];
//         private readonly Dictionary<int, ProgressCallback> _progressHandlers = [];

//         public event Action OnClose = delegate { };
//         public event Action<Exception> OnError = delegate { };

//         public Func<Request, Task<SendResultT>> FallbackRequestHandler;
//         public Func<SendNotificationT, Task> FallbackNotificationHandler;

//         protected Protocol(ITransport transport,
...
//         public void SetNotificationHandler<T>(Func<SendNotificationT, Task> handler)
//         {
//             _notificationHandlers[typeof(T).FullName!] = notification => handler(notification);
//         }
